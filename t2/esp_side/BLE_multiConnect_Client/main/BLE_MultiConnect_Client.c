...
uint8_t *rx_Adata, rx_Alen;
bool notification_AEVT = false;
//Variables to know BLE status
...
static const char remote_device_name[3][20] = {"ESP_BLE_Server_A", "ESP_BLE_Server_B", "ESP_BLE_Server_C"};
...
static void gattc_profile_a_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_OPEN_EVT:
        ...
        memcpy(gl_profile_tab[PROFILE_A_APP_ID].remote_bda, p_data->open.remote_bda, 6);// stores remote device address
        ...
        esp_err_t mtu_ret = esp_ble_gattc_send_mtu_req (gattc_if, p_data->open.conn_id); //configures the MTU size to 200 bytes.
        if (mtu_ret){
            ESP_LOGE(GATTC_TAG, "config MTU error, error code = %x", mtu_ret);
        }
        //next event :ESP_GATTC_CFG_MTU_EVT 
        break;
    case ESP_GATTC_CFG_MTU_EVT: // After configuration of the MTU size, an ESP_GATTC_CFG_MTU_EVT is generated
        ...
        esp_ble_gattc_search_service(gattc_if, param->cfg_mtu.conn_id, &remote_filter_service_uuid); //search for available known services on the remote device.
        //next event: ESP_GATTC_SEARCH_RES_EVT 
        break;
    case ESP_GATTC_SEARCH_RES_EVT: { //Once the search for all services is completed, an ESP_GATTC_SEARCH_CMPL_EVT event is generated
        ESP_LOGI(GATTC_TAG, "SEARCH RES: conn_id = %x is primary service %d", p_data->search_res.conn_id, p_data->search_res.is_primary);
        ESP_LOGI(GATTC_TAG, "start handle %d end handle %d current handle value %d", p_data->search_res.start_handle, p_data->search_res.end_handle, p_data->search_res.srvc_id.inst_id);
        if (p_data->search_res.srvc_id.uuid.len == ESP_UUID_LEN_16 && p_data->search_res.srvc_id.uuid.uuid.uuid16 == REMOTE_SERVICE_UUID) { // Comprueba característica deseada, asumo
    ...
    case ESP_GATTC_SEARCH_CMPL_EVT: // When GATT service discovery is completed, the event comes
        ...
        if (get_service_a){
            ...
            if (count > 0) {
                ...
                // next event: ESP_GATTC_REG_FOR_NOTIFY_EVT 
            }else {
                ESP_LOGE(GATTC_TAG, "no char found");
            }
        }
        break;
    case ESP_GATTC_REG_FOR_NOTIFY_EVT: { //Every time a client registers for notifications, an ESP_GATTC_REG_FOR_NOTIFY_EVT
            ...
            if (!descr_elem_result_a){
                ...
                if (count > 0 && descr_elem_result_a[0].uuid.len == ESP_UUID_LEN_16 && descr_elem_result_a[0].uuid.uuid.uuid16 == ESP_GATT_UUID_CHAR_CLIENT_CONFIG){
                    ret_status = esp_ble_gattc_write_char_descr( gattc_if,
                                                                 ...
                                                                 ESP_GATT_AUTH_REQ_NONE); //this function is used to write to characteristic descriptors
                }
    ...
    case ESP_GATTC_NOTIFY_EVT: // the remote device sends a notification which triggers the ESP_GATTC_NOTIFY_EVT
        ESP_LOGI(GATTC_TAG, "ESP_GATTC_NOTIFY_EVT, Receive notify value:");
        esp_log_buffer_hex(GATTC_TAG, p_data->notify.value, p_data->notify.value_len);
        //To save reveived data
        rx_Alen = p_data->notify.value_len;
        rx_Adata=(uint8_t*) malloc(sizeof(uint8_t) * rx_Alen);//Usa los datos escritos para luego enviarlos
        for(int i=0; i<rx_Alen; i++){
            rx_Adata[i] = p_data->notify.value[i];
        }
        notification_AEVT = true;
        break;
...
static void gattc_profile_b_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_OPEN_EVT:
        ...
        memcpy(gl_profile_tab[PROFILE_B_APP_ID].remote_bda, p_data->open.remote_bda, 6);// stores remote device address
...
static void gattc_profile_c_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_OPEN_EVT:
        ...
        memcpy(gl_profile_tab[PROFILE_C_APP_ID].remote_bda, p_data->open.remote_bda, 6);// stores remote device address
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT: {
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT:
            ...
            adv_name = esp_ble_resolve_adv_data(scan_result->scan_rst.ble_adv,
                                                ESP_BLE_AD_TYPE_NAME_CMPL, &adv_name_len); //First, the name of the device is extracted from the advertised data and stored in the adv_name
            ...
            if (adv_name != NULL) {
                ...
                if (strlen(remote_device_name[0]) == adv_name_len && strncmp((char *)adv_name, remote_device_name[0], adv_name_len) == 0) { //name comparison
                    if (conn_device_a == false) {
                        conn_device_a = true; //flag is set
                        ESP_LOGI(GATTC_TAG, "Searched device %s", remote_device_name[0]);
                        esp_ble_gap_stop_scanning();
                        esp_ble_gattc_open(gl_profile_tab[PROFILE_A_APP_ID].gattc_if, scan_result->scan_rst.bda, scan_result->scan_rst.ble_addr_type, true); //connects to the remote device
                        Isconnecting = true;
                    }
                    break;
                }
                else if (strlen(remote_device_name[1]) == adv_name_len && strncmp((char *)adv_name, remote_device_name[1], adv_name_len) == 0) { // name comparison
                    if (conn_device_b == false) {
                        conn_device_b = true; //flag is set
                        ESP_LOGI(GATTC_TAG, "Searched device %s", remote_device_name[1]);
                        esp_ble_gap_stop_scanning();
                        esp_ble_gattc_open(gl_profile_tab[PROFILE_B_APP_ID].gattc_if, scan_result->scan_rst.bda, scan_result->scan_rst.ble_addr_type, true); // //connects to the remote device
                        Isconnecting = true;

                    }
                }
                else if (strlen(remote_device_name[2]) == adv_name_len && strncmp((char *)adv_name, remote_device_name[2], adv_name_len) == 0) { //name comparison
                    if (conn_device_c == false) {
                        conn_device_c = true; //flag is set
                        ESP_LOGI(GATTC_TAG, "Searched device %s", remote_device_name[2]);
                        esp_ble_gap_stop_scanning();
                        esp_ble_gattc_open(gl_profile_tab[PROFILE_C_APP_ID].gattc_if, scan_result->scan_rst.bda, scan_result->scan_rst.ble_addr_type, true); ////connects to the remote device
                        Isconnecting = true;
                    }
                    break;
                }
...
void BLE_McClient_Init(uint16_t MTU){
    ...
    ret = esp_ble_gattc_app_register(PROFILE_A_APP_ID); //The esp_ble_gattc_app_register() function is used to register each Application Profile to the BLE stack
    if (ret){
...
/** @brief Send data through the BLE protocol as a characteristic write 
    *  @param  profile_APP_ID Profile APP ID through which to send data
...
esp_err_t BLE_send(uint8_t profile_APP_ID, uint8_t data_len, uint8_t *data){
    esp_err_t ret;
    ret = esp_ble_gattc_write_char( gl_profile_tab[profile_APP_ID].gattc_if,
